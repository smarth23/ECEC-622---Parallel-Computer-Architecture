 ...
 *           * */
	...
	gettimeofday(&stop, NULL);
	printf("CPU runtime using OpenMP= %0.2f s. \n",(float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
void gauss_eliminate_using_openmp(float* U, const float* A, unsigned int num_elements)                  /* Write code to perform gaussian elimination using OpenMP. */
{
	unsigned int i,j,k;
