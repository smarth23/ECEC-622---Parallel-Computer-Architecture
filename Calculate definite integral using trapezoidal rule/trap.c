...
int main(void)
{
    ...
    printf("CPU run time for Pthread = %0.4f s. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
void compute_using_pthreads(float a, float b, int n, float h){
    ...
    for(i=0;i<NUM_THREADS;i++){
        my_args = (trap*)malloc(sizeof(trap));
        my_args->thread_id = i;
        if(i==NUM_THREADS-1){
            my_args->start = a+(num_traps_per_thread*i)*h;
            my_args->end = b;
        }
        else {
            my_args->start = a+(num_traps_per_thread*i)*h;
            my_args->end = a+(num_traps_per_thread*(i+1))*h;
        }
        my_args->height = h;
        
        if(pthread_create(&my_thread[i],NULL,Area,(void *)my_args)!=0){
...
void * Area(void *my_args){
    trap *my_args_final = (trap *) my_args;
    int temp_id = my_args_final->thread_id;
    double i = my_args_final->start;
    double loop_end = my_args_final->end - my_args_final->height;
    double sum=0;
