...
void run_test(int num_elements) 
{
	...
	printf("CPU run time for OpenMP = %0.3f s. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
void compute_using_openmp(int *input_data, int *histogram, int num_elements, int histogram_size)
{
	...
	int histogram_private[NUM_THREADS][histogram_size];
	...
	for(i=0;i<num_elements;i++)	
	{
	histogram_private[thread_id][input_data[i]]++;
	}
	#pragma omp for 
	for(i=0;i<histogram_size;i++)
	{
	for(int j=0;j<NUM_THREADS;j++)
			histogram[i]+=histogram_private[j][i];
	}
}
